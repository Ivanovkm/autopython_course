...
class Trigon:
    """
    Класс описывает треугольник по трем сторонам
    """
    def __init__(self, *args):
        # Проверяем, все стороны - числа и они положительные:
        for arg in args:
            if type(arg) != int:
                raise TypeError('Стороны должны быть числами')
            elif arg <= 0:
                raise ValueError('Стороны должны быть положительными')
        # Проверяем количество аргументов (должно быть 3) и существование треугольника:
        if len(args) != 3:
            raise IndexError(f'Передано {len(args)} аргументов, а ожидается 3')
        elif max(args) >= sum(args) - max(args):

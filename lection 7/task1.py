...
class Segment:
    """
    Класс задает объект у которого есть две точки координат, методы позволяют
     вычислять длину отрезка и определять, пересекает ли отрезок оси абсцисс и ординат
    """
    ...
    # Метод вычисляет длину отрезка по переданным координатам его точек
    def length(self):
        return round(((self.x2 - self.x1)**2 + (self.y2 - self.y1)**2)**0.5, 2)

    # Метод определяет пересекает ли отрезок ось абсцисс

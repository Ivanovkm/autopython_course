...
def repeats(our_str):
    """
        Функция изменяет строку, добавляя к каждому символу количество раз, которое он встречался в этой строке
        :param our_str: на вход принимаем строку
        :return: возвращаем новую строку с добавленными цифрами
        """
    new_str = ''
# Формируем строку, к каждому символу добавляем _n, где n определяется по количеству вхождений
# символа в подстроку, ограниченную текущим индексом
    for char_ind in range(len(our_str)):
        new_str += our_str[char_ind] + '_' + str(our_str[0:char_ind + 1].count(our_str[char_ind]))
    return new_str

...
def everything_for_your_cat(cats_data):
    """
        Функция принимает на вход список кортежей с именами и возрастом котиков, именами их владельцев, где владельцы
        повторяются.
        Возвращает на выход данные построчно, для каждого владельца перечисляются все котики
        :param cats_data: список кортежей: [(имя котика, возраст котика, имя владельца, фамилия владельца), ...
        :return: список строк: '<Имя владельца>, <Фамилия владельца>, <Имя котика>, <Возраст котика>; <Имя котика 1>...
        """
    our_str = ''
    # Создаем и наполняем в цикле словарь для хранения
    # уникальных записей с ключом по Имя Фамилия:
    cats_dict = {}
    for elem in cats_data:
        # В ключ забираем имя и фамилию
        key = f'{elem[2]} {elem[3]}'
        # В значение - имя котика и возраст
        value = f'{elem[0]}, {elem[1]}'
        # Если ключа еще нет в словаре, добавляем новый со значением
        if key not in cats_dict:
            cats_dict[key] = value
        # Если ключ уже есть, добавляем нового котика:
        else:
            cats_dict.update({key: cats_dict[key] + '; ' + value})
    # Пишем в строку наш словарь
